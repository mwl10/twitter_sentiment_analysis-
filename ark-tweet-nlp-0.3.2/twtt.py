...
# make sure the output_ark file is empty to write to
output_ark.truncate(0)

# redirect runtagger output to the ark_output file
runTagger_shell = "./runTagger.sh --model model.ritter_ptb_alldata_fixed.20130723 {} > {}".format(sys.argv[1], sys.argv[2])
os.system(runTagger_shell)

# get the lines from the ark output
output_ark_lines = output_ark.readlines()

# make sure nothings in the tagged output file first
tagged_tweets.truncate(0)

line_index = 0
for line in output_ark_lines:
    tagged_tokens = " "
    ...
    tagged_tweets.write(sent_tag_token)
    line_index += 1
